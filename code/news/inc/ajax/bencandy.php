...
if($job=="do"){
	...
	if($step==2){
		...
		echo "<A HREF=\"$Murl/post.php?fid=$fid\">发表文章</A><br>";
		echo "<A HREF=\"$Murl/post.php?fid=$fid&id=$id\">续发文章</A><br>";
		echo "<A HREF=\"$Murl/post.php?job=edit&fid=$fid&id=$id&rid=$rid\">修改文章</A><br>";
		echo "<A HREF=\"$Murl/post.php?action=del&fid=$fid&id=$id&rid=$rid\" onclick=\"return confirm('你确认要删除吗?不可恢复');\">删除本文</A><br>";
	}
}
?>
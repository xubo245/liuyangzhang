...
if($job=="list"){
	...
	$min=($page-1)*$rows;
	$query = $db->query("SELECT C.*,A.*,B.id AS job_id,B.title,B.fid FROM {$_pre}apply A LEFT JOIN {$_pre}content B ON A.cid=B.id LEFT JOIN {$_pre}content_2 C ON A.join_id=C.id WHERE B.uid='$lfjuid' ORDER BY A.id DESC LIMIT $min,$rows");

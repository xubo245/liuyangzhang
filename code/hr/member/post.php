...
if($action=="postnew")
{
	...
	/*往标题表插入内容*/
	$db->query("INSERT INTO `{$_pre}content` (`title` , `mid` , `fid` , `fname` , `posttime` , `list` , `uid` , `username` ,  `city_id` , `yz` ,  `keywords` , `ip`  ) VALUES  ('$postdb[title]','$fidDB[mid]','$fid','$fidDB[name]','$timestamp','$timestamp','$lfjdb[uid]','$lfjdb[username]','$postdb[city_id]','$postdb[yz]','$postdb[keywords]','$onlineip')");
...
elseif($action=="edit")
{
	...
	/*更新主信息表内容*/
	$db->query("UPDATE `{$_pre}content` SET title='$postdb[title]',keywords='$postdb[keywords]',city_id='$postdb[city_id]' WHERE id='$id'");

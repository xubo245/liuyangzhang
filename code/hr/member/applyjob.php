...
if($job=="list"){
	...
	$min=($page-1)*$rows;
	$query = $db->query("SELECT A.id,B.id AS bid,B.title,B.uid,B.fid FROM {$_pre}apply A LEFT JOIN {$_pre}content B ON A.cid=B.id WHERE A.uid='$lfjuid' ORDER BY A.id DESC LIMIT $min,$rows");

...
function make_post_table($rs){	
	...
	{	
		...
		$show="<tr> <td class=tdl>{$rs[title]}:$mustfill<br>{$rs[form_title]}</td> <td class=tdr> <input class='text' type='text' name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' size='$rs[field_inputleng]' value='\$rsdb[{$rs[field_name]}]'> $rs[form_units]</td></tr>";	
	}
	elseif($rs[form_type]=='time')	
	{	
		$show="<tr> <td class=tdl>{$rs[title]}:$mustfill<br>{$rs[form_title]}</td> <td class=tdr> <input class='text'  onclick=\"setday(this,1)\" type='text' name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' size='20' value='\$rsdb[{$rs[field_name]}]'> $rs[form_units]</td></tr>";	
	}
	elseif($rs[form_type]=='upfile')	
	{	
		$show="<tr> <td class=tdl>{$rs[title]}:$mustfill<br>{$rs[form_title]}</td> <td class=tdr> <input class='text' type='text' name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' size='50' value='\$rsdb[{$rs[field_name]}]'> $rs[form_units]<br><iframe frameborder=0 height=23 scrolling=no src='\$webdb[www_url]/do/upfile.php?fn=upfile&dir=\$_pre\$fid&label=atc_{$rs[field_name]}' width=310></iframe> </td></tr>";	
	}	
	elseif($rs[form_type]=='upmorefile')	
	{	
		$show="<tr> <td class=tdl>{$rs[title]}:$mustfill<br>{$rs[form_title]}<br>增加 <input class='text' type='text' size='3' name='nums_{$rs[field_name]}' value='2'> 项 <input type='button' name='Submit2' value='增加' onClick='showinput_{$rs[field_name]}()'></td> <td class=tdr><!--	
EOT;
\$num=count(\$rsdb[{$rs[field_name]}][url]);	
\$num||\$num=1;	
for( \$i=0; \$i<\$num ;\$i++ ){	
print <<<EOT
--> 名称: <input class=\"text\" type=\"text\" name=\"postdb[{$rs[field_name]}][name][]\" id=\"atc_{$rs[field_name]}_name\$i\" size=\"15\" value=\"{\$rsdb[{$rs[field_name]}][name][\$i]}\">	
 消耗{\$webdb[MoneyName]}: <input class=\"text\" type=\"text\" name=\"postdb[{$rs[field_name]}][fen][]\" id=\"atc_{$rs[field_name]}_fen\$i\" size=\"3\" value=\"{\$rsdb[{$rs[field_name]}][fen][\$i]}\">	
 地址: 	
                    <input class=\"text\" type=\"text\" name=\"postdb[{$rs[field_name]}][url][]\" id=\"atc_{$rs[field_name]}_url\$i\" size=\"30\" value=\"{\$rsdb[{$rs[field_name]}][url][\$i]}\">	
...
function upfile_{$rs[field_name]}(url,name,size,label){	
	...
	{	
		$show="<tr><td class=tdl>{$rs[title]}:$mustfill<br>{$rs[form_title]}</td><td class=tdr><textarea name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' cols='70' rows='8'>\$rsdb[{$rs[field_name]}]</textarea>$rs[form_units]</td></tr>\n\t";	
	}	
	elseif($rs[form_type]=='ieedit')	
	{	
		$show="<tr><td class=tdl>{$rs[title]}:$mustfill<br>{$rs[form_title]}</td><td class=tdr><iframe id='eWebEditor1' src='\$webdb[www_url]/ewebeditor/ewebeditor.php?id=atc_{$rs[field_name]}&style=standard&&etype=1' frameborder='0' scrolling='no' width='90%' height='250'></iframe>$rs[form_units]<input name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' type='hidden' value='\$rsdb[{$rs[field_name]}]'></td></tr>\n\t";	
	}	
	...
	{	
		...
		$show="<tr> <td class=tdl>{$rs[title]}:$mustfill<br>{$rs[form_title]}</td><td class=tdr> <select name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}'>$_show</select>$rs[form_units]</td> </tr>\n\t";	
	}	
	...
	{	
		...
		foreach( $detail AS $key=>$value){	
			...
			list($v1,$v2)=explode("|",$value);	
			$v2 || $v2=$v1;	
			$_show.="<input class='radio' type='radio' name='postdb[{$rs[field_name]}]' value='$v1' {\$rsdb[{$rs[field_name]}]['{$v1}']}>$v2";	
		}	
		$show="<tr> <td class=tdl>{$rs[title]}:$mustfill<br>{$rs[form_title]}</td> <td class=tdr>$_show$rs[form_units]</td></tr>\n\t";	
	}	
	...
	{	
		...
		foreach( $detail AS $key=>$value){	
			...
			list($v1,$v2)=explode("|",$value);	
			$v2 || $v2=$v1;	
			$_show.="<input class='checkbox' type='checkbox' name='postdb[{$rs[field_name]}][]' value='$v1' {\$rsdb[{$rs[field_name]}]['{$v1}']}>{$v2}&nbsp;&nbsp;";	
		}	
		$show="<tr> <td class=tdl>{$rs[title]}:$mustfill<br>{$rs[form_title]}</td> <td class=tdr>$_show$rs[form_units]</td></tr>\n\t";	
	}	
	return $show;	

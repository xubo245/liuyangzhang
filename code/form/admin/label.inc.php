...
if($action=='mod'){
	...
	$SQL=" SELECT A.*,R.*,config AS M_config FROM {$_pre}content A LEFT JOIN {$_pre}content_{$amodule} R ON A.id=R.id LEFT JOIN {$_pre}module F ON A.mid=F.id $SQL ORDER BY A.id $asc  LIMIT $rows ";

...
elseif($action=='edit'&&$Apower[menu_list])
{
	...
	$db->query("UPDATE {$pre}menu SET fid='$fid',name='$postdb[name]',linkurl='$postdb[linkurl]',color='$postdb[color]',target='$postdb[target]',hide='$postdb[hide]',list='$postdb[list]',ckname='$postdb[ckname]' WHERE id='$id'");
	menu_cache();
...
function menu_cache(){
	...
	while($rs = $db->fetch_array($query)){
		$array[]="$rs[name]|$rs[linkurl]|$rs[target]|$rs[color]|$rs[id]|$rs[ckname]";
